...
type MailStatsVolume* = object
  ...
  to*: Option[Table[string, int]] ## Message counts keyed by destination (envelope `to`) email address.
  `from`*: Option[Table[string, int]] ## Message counts keyed by sender (envelope `from`) email address.
  ip*: Option[Table[string, int]] ## Message counts keyed by originating client IP address.
