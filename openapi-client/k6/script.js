...
export default function() {
    ...
    group("/mail/log", () => {
        ...
        let delivered = 'TODO_EDIT_THE_DELIVERED'; // specify value as there is no example value for this parameter in OpenAPI spec
        ...
        {
            let url = BASE_URL + `/mail/log?id=${id}&origin=${origin}&mx=${mx}&from=${from}&to=${to}&subject=${subject}&mailid=${mailid}&skip=${skip}&limit=${limit}&startDate=${startDate}&endDate=${endDate}&replyto=${replyto}&headerfrom=${headerfrom}&delivered=${delivered}`;
            let params = {
                headers: {
                    "X-API-KEY": `${X_API_KEY}`, "Accept": "application/json"
                }
    ...
    group("/mail/rawsend", () => {
        ...
        // Request No. 1: rawMail
        {
            ...
            check(request, {
                "successful email response": (r) => r.status === 200
            });
        }
    });
