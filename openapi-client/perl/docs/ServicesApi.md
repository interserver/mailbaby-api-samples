...
> MailOrderDetail get_mail_order_by_id(id => $id)
...
my $id = 39; # int | The numeric ID of the mail order.

eval {
    my $result = $api_instance->get_mail_order_by_id(id => $id);
    print Dumper($result);
};
if ($@) {
    warn "Exception when calling ServicesApi->get_mail_order_by_id: $@\n";
}
```
