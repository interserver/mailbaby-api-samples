...
class Decoders {
    ...
    private static var __once: () = {
        ...
        // Decoder for [MailContact]
        Decoders.addDecoder(clazz: [MailContact].self) { (source: AnyObject, instance: AnyObject?) -> Decoded<[MailContact]> in
            return Decoders.decode(clazz: [MailContact].self, source: source)
        }

        // Decoder for MailContact
        Decoders.addDecoder(clazz: MailContact.self) { (source: AnyObject, instance: AnyObject?) -> Decoded<MailContact> in
            if let sourceDictionary = source as? [AnyHashable: Any] {
                let _result = instance == nil ? MailContact() : instance as! MailContact
                ...
                return .failure(.typeMismatch(expected: "MailContact", actual: "\(source)"))
            }
        }
        // Decoder for [MailLog]
        ...
        Decoders.addDecoder(clazz: SendMailAdv.self) { (source: AnyObject, instance: AnyObject?) -> Decoded<SendMailAdv> in
            if let sourceDictionary = source as? [AnyHashable: Any] {
                ...
                switch Decoders.decodeOptional(clazz: [MailContact].self, source: sourceDictionary["to"] as AnyObject?) {
                case let .success(value): _result.to = value
                case let .failure(error): break
                }
                switch Decoders.decodeOptional(clazz: Int64.self, source: sourceDictionary["id"] as AnyObject?) {
                ...
                switch Decoders.decodeOptional(clazz: [MailContact].self, source: sourceDictionary["cc"] as AnyObject?) {
                case let .success(value): _result.cc = value
                case let .failure(error): break
                }
                switch Decoders.decodeOptional(clazz: [MailContact].self, source: sourceDictionary["bcc"] as AnyObject?) {
