...
class BlockingApi(
   ...
   * @return GenericResponse
   */
  def delistBlock(body: EmailAddressParam, email: String): Option[GenericResponse] = {
   ...
   * @return Future(GenericResponse)
   */
  def delistBlockAsync(body: EmailAddressParam, email: String): Future[GenericResponse] = {
...
class BlockingApiAsyncHelper(client: TransportClient, config: SwaggerConfig) extends ApiClient(client, config) {
  ...
  def delistBlock(body: EmailAddressParam,
    email: String)(implicit reader: ClientResponseReader[GenericResponse], writer: RequestWriter[EmailAddressParam]): Future[GenericResponse] = {
    // create path and map variables
