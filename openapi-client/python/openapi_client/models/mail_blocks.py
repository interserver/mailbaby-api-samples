...
class MailBlocks(BaseModel):
    ...
    """ # noqa: E501
    local: List[MailBlockClickHouse] = Field(description="Messages flagged by the `LOCAL_BL_RCPT` rspamd rule in the last 5 days. These are messages sent to recipients on a local block list.")
    mbtrap: List[MailBlockClickHouse] = Field(description="Messages flagged by the `MBTRAP` rspamd rule in the last 5 days. These triggered MailBaby's honeypot / trap address detection.")
    subject: List[MailBlockRspamd] = Field(description="Senders whose messages contained spam-indicative subjects (containing `@`, `smtp`, `socks4`, or `socks5`) with more than 4 occurrences of the same subject in the last 3 days.")
