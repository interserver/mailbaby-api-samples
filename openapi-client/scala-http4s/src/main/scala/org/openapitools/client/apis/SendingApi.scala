...
trait SendingApiEndpoints[F[*]] {
  ...
  def sendMail(to: SendMailTo, from: String, subject: String, body: String, id: Option[Long] = None)(using auth: _Authorization.ApiKey): F[GenericResponse]
...
class SendingApiEndpointsImpl[F[*]: Concurrent](
  ...
  override def sendMail(to: SendMailTo, from: String, subject: String, body: String, id: Option[Long] = None)(using auth: _Authorization.ApiKey): F[GenericResponse] = {
    val requestHeaders = Seq(
      Some("Content-Type" -> "application/x-www-form-urlencoded")
    ).flatten
    val formParameters = Some((
