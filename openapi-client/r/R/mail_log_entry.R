...
#' @field _id Internal auto-increment database row ID.  Not meaningful outside the API. integer
#' @field id The relay-assigned mail ID (18–19 hex characters).  This is the value returned as `text` by the sending endpoints and accepted as the `mailid` filter on `GET /mail/log`. character
#' @field from SMTP envelope `MAIL FROM` address (may differ from the `From:` header). character
#' @field to SMTP envelope `RCPT TO` address. character
#' @field subject The `Subject` header value, if available. character [optional]
#' @field messageId The `Message-ID` header value, if present.  Can be used with the `messageId` filter on `GET /mail/log` for subsequent lookups. character [optional]
#' @field created Human-readable creation timestamp in `YYYY-MM-DD HH:MM:SS` format. character
#' @field time Unix timestamp of message acceptance.  Corresponds to the `startDate` and `endDate` filter parameters on `GET /mail/log`. integer
#' @field user The SMTP AUTH username used to submit the message (e.g. `mb5658`). Corresponds to the `username` field in `GET /mail` orders. character
#' @field transtype SMTP transaction type negotiated with the relay (e.g. `ESMTPSA`). character
#' @field origin IP address of the client that submitted the message to the relay. Corresponds to the `origin` filter parameter on `GET /mail/log`. character
#' @field interface Relay interface name that accepted the message (e.g. `feeder`). character
...
#' @field delivered Delivery status flag.  `1` = successfully delivered to destination MX. `0` = queued, deferred, or failed.  `null` = delivery not yet attempted. Corresponds to the `delivered` filter parameter on `GET /mail/log`. integer [optional]
#' @field response The SMTP response string received from the destination MX server upon delivery attempt (e.g. `\"250 2.0.0 Ok queued as C91D83E128C\"`). character [optional]
#' @field recipient The specific recipient address this delivery record is for. character [optional]
#' @field domain The destination domain.  Corresponds to the `mx` filter parameter (which matches `mxHostname`, not `domain`) on `GET /mail/log`. character [optional]
...
#' @field mxHostname The MX hostname the relay connected to for delivery.  Corresponds to the `mx` filter parameter on `GET /mail/log`. character [optional]
...
MailLogEntry <- R6::R6Class(
  ...
  public = list(
    ...
    initialize = function(`_id`, `id`, `from`, `to`, `created`, `time`, `user`, `transtype`, `origin`, `interface`, `subject` = NULL, `messageId` = NULL, `sendingZone` = NULL, `bodySize` = NULL, `seq` = NULL, `delivered` = NULL, `response` = NULL, `recipient` = NULL, `domain` = NULL, `locked` = NULL, `lockTime` = NULL, `assigned` = NULL, `queued` = NULL, `mxHostname` = NULL, ...) {
      if (!missing(`_id`)) {
