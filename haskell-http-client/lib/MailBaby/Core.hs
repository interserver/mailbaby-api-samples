...
-- | 
data MailBabyConfig = MailBabyConfig
...
-- | display the config
instance P.Show MailBabyConfig where
  show c =
    T.printf
...
-- | constructs a default MailBabyConfig
--
-- configHost:
--
...
-- @"mail-baby/0.1.0.0"@
--
newConfig :: IO MailBabyConfig
newConfig = do
    logCxt <- initLogContext
    return $ MailBabyConfig
        { configHost = "https://api.mailbaby.net"
        , configUserAgent = "mail-baby/0.1.0.0"
...
addAuthMethod :: AuthMethod auth => MailBabyConfig -> auth -> MailBabyConfig
addAuthMethod config@MailBabyConfig {configAuthMethods = as} a =
  config { configAuthMethods = AnyAuthMethod a : as}

-- | updates the config to use stdout logging
withStdoutLogging :: MailBabyConfig -> IO MailBabyConfig
...
withStderrLogging :: MailBabyConfig -> IO MailBabyConfig
...
withNoLogging :: MailBabyConfig -> MailBabyConfig
...
data MailBabyRequest req contentType res accept = MailBabyRequest
  { rMethod  :: NH.Method   -- ^ Method of MailBabyRequest
  , rUrlPath :: [BCL.ByteString] -- ^ Endpoint of MailBabyRequest
  , rParams   :: Params -- ^ params of MailBabyRequest
  , rAuthTypes :: [P.TypeRep] -- ^ types of auth methods
  }
  deriving (P.Show)

-- | 'rMethod' Lens
rMethodL :: Lens_' (MailBabyRequest req contentType res accept) NH.Method
rMethodL f MailBabyRequest{..} = (\rMethod -> MailBabyRequest { rMethod, ..} ) <$> f rMethod
{-# INLINE rMethodL #-}

-- | 'rUrlPath' Lens
rUrlPathL :: Lens_' (MailBabyRequest req contentType res accept) [BCL.ByteString]
rUrlPathL f MailBabyRequest{..} = (\rUrlPath -> MailBabyRequest { rUrlPath, ..} ) <$> f rUrlPath
{-# INLINE rUrlPathL #-}

-- | 'rParams' Lens
rParamsL :: Lens_' (MailBabyRequest req contentType res accept) Params
rParamsL f MailBabyRequest{..} = (\rParams -> MailBabyRequest { rParams, ..} ) <$> f rParams
{-# INLINE rParamsL #-}

-- | 'rParams' Lens
rAuthTypesL :: Lens_' (MailBabyRequest req contentType res accept) [P.TypeRep]
rAuthTypesL f MailBabyRequest{..} = (\rAuthTypes -> MailBabyRequest { rAuthTypes, ..} ) <$> f rAuthTypes
{-# INLINE rAuthTypesL #-}
...
class HasBodyParam req param where
  setBodyParam :: forall contentType res accept. (Consumes req contentType, MimeRender contentType param) => MailBabyRequest req contentType res accept -> param -> MailBabyRequest req contentType res accept
  setBodyParam req xs =
...
class HasOptionalParam req param where
  ...
  applyOptionalParam :: MailBabyRequest req contentType res accept -> param -> MailBabyRequest req contentType res accept
  applyOptionalParam = (-&-)
  {-# INLINE applyOptionalParam #-}

  -- | infix operator \/ alias for 'addOptionalParam'
  (-&-) :: MailBabyRequest req contentType res accept -> param -> MailBabyRequest req contentType res accept
  (-&-) = applyOptionalParam
  {-# INLINE (-&-) #-}
...
-- ** MailBabyRequest Utils

_mkRequest :: NH.Method -- ^ Method 
          -> [BCL.ByteString] -- ^ Endpoint
          -> MailBabyRequest req contentType res accept -- ^ req: Request Type, res: Response Type
_mkRequest m u = MailBabyRequest m u _mkParams []

_mkParams :: Params
_mkParams = Params [] [] ParamBodyNone

setHeader ::
     MailBabyRequest req contentType res accept
  -> [NH.Header]
  -> MailBabyRequest req contentType res accept
...
removeHeader :: MailBabyRequest req contentType res accept -> [NH.HeaderName] -> MailBabyRequest req contentType res accept
...
_setContentTypeHeader :: forall req contentType res accept. MimeType contentType => MailBabyRequest req contentType res accept -> MailBabyRequest req contentType res accept
_setContentTypeHeader req =
    case mimeType (P.Proxy :: P.Proxy contentType) of 
        Just m -> req `setHeader` [("content-type", BC.pack $ P.show m)]
        Nothing -> req `removeHeader` ["content-type"]

_setAcceptHeader :: forall req contentType res accept. MimeType accept => MailBabyRequest req contentType res accept -> MailBabyRequest req contentType res accept
...
addForm :: MailBabyRequest req contentType res accept -> WH.Form -> MailBabyRequest req contentType res accept
addForm req newform = 
...
_addMultiFormPart :: MailBabyRequest req contentType res accept -> NH.Part -> MailBabyRequest req contentType res accept
...
_setBodyBS :: MailBabyRequest req contentType res accept -> B.ByteString -> MailBabyRequest req contentType res accept
_setBodyBS req body = 
    req & L.set (rParamsL . paramsBodyL) (ParamBodyB body)

_setBodyLBS :: MailBabyRequest req contentType res accept -> BL.ByteString -> MailBabyRequest req contentType res accept
_setBodyLBS req body = 
    req & L.set (rParamsL . paramsBodyL) (ParamBodyBL body)

_hasAuthType :: AuthMethod authMethod => MailBabyRequest req contentType res accept -> P.Proxy authMethod -> MailBabyRequest req contentType res accept
...
class P.Typeable a =>
  ...
  applyAuthMethod
    ...
    -> a
    -> MailBabyRequest req contentType res accept
    -> IO (MailBabyRequest req contentType res accept)
...
_applyAuthMethods
  ...
  -> MailBabyConfig
  -> IO (MailBabyRequest req contentType res accept)
_applyAuthMethods req config@(MailBabyConfig {configAuthMethods = as}) =
  foldlM go req as
  where
