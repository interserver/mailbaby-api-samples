...
object MailOrderDetail {
  given encoderMailOrderDetail: Encoder[MailOrderDetail] = Encoder.instance { t =>
    Json.fromFields{
      Seq(
        Some("id" -> t.id.asJson),
  ...
  given decoderMailOrderDetail: Decoder[MailOrderDetail] = Decoder.instance { c =>
    for {
