...
```php
sendAdvMailById($id, $send_mail): \OpenAPI\Client\Model\GenericResponse
```
...
try {
    $result = $apiInstance->sendAdvMailById($id, $send_mail);
    print_r($result);
} catch (Exception $e) {
...
```php
sendMailById($id, $subject, $body, $to, $to_name, $from, $from_name): \OpenAPI\Client\Model\GenericResponse
```
