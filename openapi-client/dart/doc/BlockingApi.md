...
final api_instance = BlockingApi();
final ruleId = 34; // int | The numeric ID of the deny rule to delete.  Obtain this from the `id` field in `GET /mail/rules` or the `text` field of a `POST /mail/rules` response.
...
final api_instance = BlockingApi();
final emailAddressParam = EmailAddressParam(); // EmailAddressParam | 

try {
    final result = api_instance.delistBlock(emailAddressParam);
    print(result);
} catch (e) {
