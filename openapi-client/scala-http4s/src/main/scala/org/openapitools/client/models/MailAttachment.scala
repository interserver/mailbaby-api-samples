...
object MailAttachment {
  given encoderMailAttachment: Encoder[MailAttachment] = Encoder.instance { t =>
    Json.fromFields{
      Seq(
        Some("filename" -> t.filename.asJson),
        Some("data" -> t.data.asJson)
      ).flatten
    }
  }
  given decoderMailAttachment: Decoder[MailAttachment] = Decoder.instance { c =>
    for {
      filename <- c.downField("filename").as[String]
      data <- c.downField("data").as[ByteArray]
