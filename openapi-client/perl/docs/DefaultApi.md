...
# **send_mail_by_id**
> GenericResponse send_mail_by_id(id => $id, subject => $subject, body => $body, to => $to, from => $from)
...
eval { 
    my $result = $api_instance->send_mail_by_id(id => $id, subject => $subject, body => $body, to => $to, from => $from);
    print Dumper($result);
};
if ($@) {
