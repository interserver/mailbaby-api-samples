...
object GetStats200ResponseInner {
  given encoderGetStats200ResponseInner: Encoder[GetStats200ResponseInner] = Encoder.instance { t =>
    Json.fromFields{
      Seq(
        Some("id" -> t.id.asJson),
  ...
  given decoderGetStats200ResponseInner: Decoder[GetStats200ResponseInner] = Decoder.instance { c =>
    for {
    ...
    } yield GetStats200ResponseInner(
      id = id,
      status = status,
      username = username,
      password = password,
