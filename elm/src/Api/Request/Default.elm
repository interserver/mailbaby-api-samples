...
-}
sendAdvMailById : Int -> Api.Data.SendMail -> Api.Request Api.Data.GenericResponse
sendAdvMailById id_path sendMail_body =
    Api.request
        "POST"
        "/mail/{id}/advsend"
        [ ( "id", String.fromInt id_path ) ]
        []
        []
        (Just (Api.Data.encodeSendMail sendMail_body))
...
-}
sendMailById : Int -> Maybe String -> Maybe String -> Maybe String -> Maybe String -> Maybe String -> Maybe String -> Api.Request Api.Data.GenericResponse
sendMailById id_path subject_query body_query to_query toName_query from_query fromName_query =
    Api.request
        "POST"
        "/mail/{id}/send"
        [ ( "id", String.fromInt id_path ) ]
        [ ( "subject", Maybe.map identity subject_query ), ( "body", Maybe.map identity body_query ), ( "to", Maybe.map identity to_query ), ( "toName", Maybe.map identity toName_query ), ( "from", Maybe.map identity from_query ), ( "fromName", Maybe.map identity fromName_query ) ]
        []
        Nothing
        Api.Data.genericResponseDecoder
