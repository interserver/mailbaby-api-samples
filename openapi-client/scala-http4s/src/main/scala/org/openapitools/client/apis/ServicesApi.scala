...
trait ServicesApiEndpoints[F[*]] {
  ...
  def getMailOrderById(id: Long)(using auth: _Authorization.ApiKey): F[MailOrderDetail]
...
class ServicesApiEndpointsImpl[F[*]: Concurrent](
  ...
  override def getMailOrderById(id: Long)(using auth: _Authorization.ApiKey): F[MailOrderDetail] = {
    val requestHeaders = Seq(
      Some("Content-Type" -> "application/json")
    ).flatten

    _executeRequest[Unit, MailOrderDetail](
      method = "GET",
      path = s"/mail/${id}",
      body = None,
      formParameters = None,
      ...
      auth = Some(auth)) {
        ...
        case r if r.status.code == 200 => parseJson[F, MailOrderDetail]("MailOrderDetail", r)
