...
async function example() {
  ...
  const body = {
    // 'all' | 'billing' | 'month' | '7d' | '24h' | 'day' | '1h' | The time window to scope `received`, `sent`, and `volume` statistics. Does not affect `usage` or `cost`, which are always calculated over the current billing cycle.  Defaults to `1h`. (optional)
    time: time_example,
...
|------------- | ------------- | ------------- | -------------|
| **time** | `all`, `billing`, `month`, `7d`, `24h`, `day`, `1h` | The time window to scope &#x60;received&#x60;, &#x60;sent&#x60;, and &#x60;volume&#x60; statistics. Does not affect &#x60;usage&#x60; or &#x60;cost&#x60;, which are always calculated over the current billing cycle.  Defaults to &#x60;1h&#x60;. | [Optional] [Defaults to `&#39;1h&#39;`] [Enum: all, billing, month, 7d, 24h, day, 1h] |
...
async function example() {
  ...
  const body = {
    ...
    to: you@receiver.com,
    // string | Filter by email `Subject` header (exact match).  To search for a substring, include it in the full subject text. (optional)
    ...
    // string | Filter by the `Message-ID` email header using a substring (case-insensitive) match.  The `Message-ID` is assigned by the sending mail client and is visible in the `messageId` field of `MailLogEntry`.  Useful when you know the message ID generated by your application but not the relay `mailid`. (optional)
    ...
    limit: 100,
    // number | Earliest date to include, as a Unix timestamp (seconds since epoch). Messages with a `time` value **greater than or equal to** this value will be included. (optional)
    startDate: 1641781008,
    // number | Latest date to include, as a Unix timestamp (seconds since epoch). Messages with a `time` value **less than or equal to** this value will be included. (optional)
    endDate: 1673317008,
