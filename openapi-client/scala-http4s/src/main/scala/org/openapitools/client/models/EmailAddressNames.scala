...
object EmailAddressNames {
  given encoderEmailAddressNames: Encoder[EmailAddressNames] = Encoder.instance { t =>
    Json.fromFields{
      Seq(
      ).flatten
    }
  }
  given decoderEmailAddressNames: Decoder[EmailAddressNames] = Decoder.instance { c =>
    for {
    } yield EmailAddressNames(
    )
  }
}
