/**
 * MailBaby Email Delivery API
 * **Send emails fast and with confidence through our easy to use [REST](https://en.wikipedia.org/wiki/Representational_state_transfer) API interface.**   # 📌 Overview  This is the API interface to the [Mail Baby](https//mail.baby/) Mail services provided by [InterServer](https://www.interserver.net). To use this service you must have an account with us at [my.interserver.net](https://my.interserver.net).   # 🔐 Authentication  In order to use most of the API calls you must pass credentials from the [my.interserver.net](https://my.interserver.net/) site.  We support several different authentication methods but the preferred method is to use the **API Key** which you can get from the [Account Security](https://my.interserver.net/account_security) page. 
 *
 * The version of the OpenAPI document: 1.0.1
 * Contact: support@interserver.net
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

/*
 * OAIMailLogEntry.h
 *
 * An email record
 */

#ifndef OAIMailLogEntry_H
#define OAIMailLogEntry_H

#include <QJsonObject>

#include <QString>

#include "OAIEnum.h"
#include "OAIObject.h"

namespace OpenAPI {

class OAIMailLogEntry : public OAIObject {
public:
    OAIMailLogEntry();
    OAIMailLogEntry(QString json);
    ~OAIMailLogEntry() override;

    QString asJson() const override;
    QJsonObject asJsonObject() const override;
    void fromJsonObject(QJsonObject json) override;
    void fromJson(QString jsonString) override;

    qint32 getId() const;
    void setId(const qint32 &_id);
    bool is__id_Set() const;
    bool is__id_Valid() const;

    QString getId() const;
    void setId(const QString &id);
    bool is_id_Set() const;
    bool is_id_Valid() const;

    QString getFrom() const;
    void setFrom(const QString &from);
    bool is_from_Set() const;
    bool is_from_Valid() const;

    QString getTo() const;
    void setTo(const QString &to);
    bool is_to_Set() const;
    bool is_to_Valid() const;

    QString getSubject() const;
    void setSubject(const QString &subject);
    bool is_subject_Set() const;
    bool is_subject_Valid() const;

    QString getMessageId() const;
    void setMessageId(const QString &message_id);
    bool is_message_id_Set() const;
    bool is_message_id_Valid() const;

    QString getCreated() const;
    void setCreated(const QString &created);
    bool is_created_Set() const;
    bool is_created_Valid() const;

    qint32 getTime() const;
    void setTime(const qint32 &time);
    bool is_time_Set() const;
    bool is_time_Valid() const;

    QString getUser() const;
    void setUser(const QString &user);
    bool is_user_Set() const;
    bool is_user_Valid() const;

    QString getTranstype() const;
    void setTranstype(const QString &transtype);
    bool is_transtype_Set() const;
    bool is_transtype_Valid() const;

    QString getTranshost() const;
    void setTranshost(const QString &transhost);
    bool is_transhost_Set() const;
    bool is_transhost_Valid() const;

    QString getOriginhost() const;
    void setOriginhost(const QString &originhost);
    bool is_originhost_Set() const;
    bool is_originhost_Valid() const;

    QString getOrigin() const;
    void setOrigin(const QString &origin);
    bool is_origin_Set() const;
    bool is_origin_Valid() const;

    QString getInterface() const;
    void setInterface(const QString &interface);
    bool is_interface_Set() const;
    bool is_interface_Valid() const;

    QString getDate() const;
    void setDate(const QString &date);
    bool is_date_Set() const;
    bool is_date_Valid() const;

    QString getSendingZone() const;
    void setSendingZone(const QString &sending_zone);
    bool is_sending_zone_Set() const;
    bool is_sending_zone_Valid() const;

    qint32 getBodySize() const;
    void setBodySize(const qint32 &body_size);
    bool is_body_size_Set() const;
    bool is_body_size_Valid() const;

    QString getSourceMd5() const;
    void setSourceMd5(const QString &source_md5);
    bool is_source_md5_Set() const;
    bool is_source_md5_Valid() const;

    qint32 getSeq() const;
    void setSeq(const qint32 &seq);
    bool is_seq_Set() const;
    bool is_seq_Valid() const;

    QString getDomain() const;
    void setDomain(const QString &domain);
    bool is_domain_Set() const;
    bool is_domain_Valid() const;

    QString getRecipient() const;
    void setRecipient(const QString &recipient);
    bool is_recipient_Set() const;
    bool is_recipient_Valid() const;

    qint32 getLocked() const;
    void setLocked(const qint32 &locked);
    bool is_locked_Set() const;
    bool is_locked_Valid() const;

    qint32 getLockTime() const;
    void setLockTime(const qint32 &lock_time);
    bool is_lock_time_Set() const;
    bool is_lock_time_Valid() const;

    QString getAssigned() const;
    void setAssigned(const QString &assigned);
    bool is_assigned_Set() const;
    bool is_assigned_Valid() const;

    QString getQueued() const;
    void setQueued(const QString &queued);
    bool is_queued_Set() const;
    bool is_queued_Valid() const;

    QString getLock() const;
    void setLock(const QString &_lock);
    bool is__lock_Set() const;
    bool is__lock_Valid() const;

    QString getLogger() const;
    void setLogger(const QString &logger);
    bool is_logger_Set() const;
    bool is_logger_Valid() const;

    qint32 getMxPort() const;
    void setMxPort(const qint32 &mx_port);
    bool is_mx_port_Set() const;
    bool is_mx_port_Valid() const;

    QString getConnectionKey() const;
    void setConnectionKey(const QString &connection_key);
    bool is_connection_key_Set() const;
    bool is_connection_key_Valid() const;

    QString getMxHostname() const;
    void setMxHostname(const QString &mx_hostname);
    bool is_mx_hostname_Set() const;
    bool is_mx_hostname_Valid() const;

    QString getSentBodyHash() const;
    void setSentBodyHash(const QString &sent_body_hash);
    bool is_sent_body_hash_Set() const;
    bool is_sent_body_hash_Valid() const;

    qint32 getSentBodySize() const;
    void setSentBodySize(const qint32 &sent_body_size);
    bool is_sent_body_size_Set() const;
    bool is_sent_body_size_Valid() const;

    qint32 getMd5Match() const;
    void setMd5Match(const qint32 &md5_match);
    bool is_md5_match_Set() const;
    bool is_md5_match_Valid() const;

    virtual bool isSet() const override;
    virtual bool isValid() const override;

private:
    void initializeModel();

    qint32 _id;
    bool m__id_isSet;
    bool m__id_isValid;

    QString id;
    bool m_id_isSet;
    bool m_id_isValid;

    QString from;
    bool m_from_isSet;
    bool m_from_isValid;

    QString to;
    bool m_to_isSet;
    bool m_to_isValid;

    QString subject;
    bool m_subject_isSet;
    bool m_subject_isValid;

    QString message_id;
    bool m_message_id_isSet;
    bool m_message_id_isValid;

    QString created;
    bool m_created_isSet;
    bool m_created_isValid;

    qint32 time;
    bool m_time_isSet;
    bool m_time_isValid;

    QString user;
    bool m_user_isSet;
    bool m_user_isValid;

    QString transtype;
    bool m_transtype_isSet;
    bool m_transtype_isValid;

    QString transhost;
    bool m_transhost_isSet;
    bool m_transhost_isValid;

    QString originhost;
    bool m_originhost_isSet;
    bool m_originhost_isValid;

    QString origin;
    bool m_origin_isSet;
    bool m_origin_isValid;

    QString interface;
    bool m_interface_isSet;
    bool m_interface_isValid;

    QString date;
    bool m_date_isSet;
    bool m_date_isValid;

    QString sending_zone;
    bool m_sending_zone_isSet;
    bool m_sending_zone_isValid;

    qint32 body_size;
    bool m_body_size_isSet;
    bool m_body_size_isValid;

    QString source_md5;
    bool m_source_md5_isSet;
    bool m_source_md5_isValid;

    qint32 seq;
    bool m_seq_isSet;
    bool m_seq_isValid;

    QString domain;
    bool m_domain_isSet;
    bool m_domain_isValid;

    QString recipient;
    bool m_recipient_isSet;
    bool m_recipient_isValid;

    qint32 locked;
    bool m_locked_isSet;
    bool m_locked_isValid;

    qint32 lock_time;
    bool m_lock_time_isSet;
    bool m_lock_time_isValid;

    QString assigned;
    bool m_assigned_isSet;
    bool m_assigned_isValid;

    QString queued;
    bool m_queued_isSet;
    bool m_queued_isValid;

    QString _lock;
    bool m__lock_isSet;
    bool m__lock_isValid;

    QString logger;
    bool m_logger_isSet;
    bool m_logger_isValid;

    qint32 mx_port;
    bool m_mx_port_isSet;
    bool m_mx_port_isValid;

    QString connection_key;
    bool m_connection_key_isSet;
    bool m_connection_key_isValid;

    QString mx_hostname;
    bool m_mx_hostname_isSet;
    bool m_mx_hostname_isValid;

    QString sent_body_hash;
    bool m_sent_body_hash_isSet;
    bool m_sent_body_hash_isValid;

    qint32 sent_body_size;
    bool m_sent_body_size_isSet;
    bool m_sent_body_size_isValid;

    qint32 md5_match;
    bool m_md5_match_isSet;
    bool m_md5_match_isValid;
};

} // namespace OpenAPI

Q_DECLARE_METATYPE(OpenAPI::OAIMailLogEntry)

#endif // OAIMailLogEntry_H
