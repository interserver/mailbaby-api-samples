...
let body = new MailBabyEmailDeliveryAndManagementServiceApi.DenyRuleNew(); // DenyRuleNew | Fields required to create a new email deny rule.
...
let apiInstance = new MailBabyEmailDeliveryAndManagementServiceApi.BlockingApi();
let ruleId = 56; // Number | The numeric ID of the deny rule to delete.  Obtain this from the `id` field in `GET /mail/rules` or the `text` field of a `POST /mail/rules` response.
...
let apiInstance = new MailBabyEmailDeliveryAndManagementServiceApi.BlockingApi();
let body = new MailBabyEmailDeliveryAndManagementServiceApi.EmailAddressParam(); // EmailAddressParam | 
