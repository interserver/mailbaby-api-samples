...
namespace Example
{
    public class GetStatsExample
    {
        public void main()
        {
            ...
            var time = time_example;  // string | The time window to scope `received`, `sent`, and `volume` statistics. Does not affect `usage` or `cost`, which are always calculated over the current billing cycle.  Defaults to `1h`. (optional)  (default to 1h)
...
namespace Example
{
    public class ViewMailLogExample
    {
        public void main()
        {
            ...
            var subject = subject_example;  // string | Filter by email `Subject` header (exact match).  To search for a substring, include it in the full subject text. (optional) 
            ...
            var startDate = 789;  // long? | Earliest date to include, as a Unix timestamp (seconds since epoch). Messages with a `time` value **greater than or equal to** this value will be included. (optional) 
            var endDate = 789;  // long? | Latest date to include, as a Unix timestamp (seconds since epoch). Messages with a `time` value **less than or equal to** this value will be included. (optional) 
