...
object EmailAddressParam {
  given encoderEmailAddressParam: Encoder[EmailAddressParam] = Encoder.instance { t =>
    Json.fromFields{
      Seq(
  ...
  given decoderEmailAddressParam: Decoder[EmailAddressParam] = Decoder.instance { c =>
    for {
      email <- c.downField("email").as[String]
    } yield EmailAddressParam(
      email = email
    )
  }
}
