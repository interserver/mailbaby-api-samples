...
function default_api:view_mail_log(id, search, skip, limit)
	local req = http_request.new_from_uri({
		scheme = self.default_scheme;
		host = self.host;
		port = self.port;
		path = string.format("%s/mail/log?id=%s&search=%s&skip=%s&limit=%s",
			self.basePath, http_util.encodeURIComponent(id), http_util.encodeURIComponent(search), http_util.encodeURIComponent(skip), http_util.encodeURIComponent(limit));
	})
