...
# **sendMail**
```swift
    open class func sendMail(body: Body, completion: @escaping (_ data: GenericResponse?, _ error: Error?) -> Void)
```
...
let body = body(to: "to_example", from: "from_example", subject: "subject_example", body: "body_example") // Body | 

// Sends an Email
DefaultAPI.sendMail(body: body) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
