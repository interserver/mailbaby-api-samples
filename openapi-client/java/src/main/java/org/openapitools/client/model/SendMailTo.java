...
public class SendMailTo extends AbstractOpenApiSchema {
    private static final Logger log = Logger.getLogger(SendMailTo.class.getName());

    public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
        @SuppressWarnings("unchecked")
        @Override
        public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
            if (!SendMailTo.class.isAssignableFrom(type.getRawType())) {
                return null; // this class only serializes 'SendMailTo' and its subtypes
            }
            ...
            return (TypeAdapter<T>) new TypeAdapter<SendMailTo>() {
                @Override
                public void write(JsonWriter out, SendMailTo value) throws IOException {
                ...
                @Override
                public SendMailTo read(JsonReader in) throws IOException {
                    ...
                    if (match == 1) {
                        SendMailTo ret = new SendMailTo();
                        ret.setActualInstance(actualAdapter.fromJsonTree(jsonElement));
                        return ret;
                    }

                    throw new IOException(String.format(java.util.Locale.ROOT, "Failed deserialization for SendMailTo: %d classes match result, expected 1. Detailed failure message for oneOf schemas: %s. JSON: %s", match, errorMessages, jsonElement.toString()));
                }
            }.nullSafe();
        }
    }
     ...
     * @throws IOException if the JSON Element is invalid with respect to SendMailTo
     */
    public static void validateJsonElement(JsonElement jsonElement) throws IOException {
        ...
        if (validCount != 1) {
            throw new IOException(String.format(java.util.Locale.ROOT, "The JSON string is invalid for SendMailTo with oneOf schemas: List<String>, String. %d class(es) match the result, expected 1. Detailed failure message for oneOf schemas: %s. JSON: %s", validCount, errorMessages, jsonElement.toString()));
        }
    }

    /**
     * Create an instance of SendMailTo given an JSON string
     *
     * @param jsonString JSON string
     * @return An instance of SendMailTo
     * @throws IOException if the JSON string is invalid with respect to SendMailTo
     */
    public static SendMailTo fromJson(String jsonString) throws IOException {
        return JSON.getGson().fromJson(jsonString, SendMailTo.class);
    }

    /**
     * Convert an instance of SendMailTo to an JSON string
     *
     * @return JSON string
     */
