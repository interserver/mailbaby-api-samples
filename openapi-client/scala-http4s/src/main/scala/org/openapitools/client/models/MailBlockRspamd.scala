...
object MailBlockRspamd {
  given encoderMailBlockRspamd: Encoder[MailBlockRspamd] = Encoder.instance { t =>
    Json.fromFields{
      Seq(
  ...
  given decoderMailBlockRspamd: Decoder[MailBlockRspamd] = Decoder.instance { c =>
    for {
      from <- c.downField("from").as[String]
      subject <- c.downField("subject").as[String]
    } yield MailBlockRspamd(
      from = from,
      subject = subject
    )
  }
