...
object GenericResponse {
  given encoderGenericResponse: Encoder[GenericResponse] = Encoder.instance { t =>
    Json.fromFields{
      Seq(
  ...
  given decoderGenericResponse: Decoder[GenericResponse] = Decoder.instance { c =>
    for {
      status <- c.downField("status").as[Option[String]]
      text <- c.downField("text").as[Option[String]]
    } yield GenericResponse(
      status = status,
      text = text
    )
  }
