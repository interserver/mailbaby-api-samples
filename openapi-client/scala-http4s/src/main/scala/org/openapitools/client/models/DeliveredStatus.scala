...
/** 
  */
enum DeliveredStatus(val value: String) {
  case `0` extends DeliveredStatus("0")
  case `1` extends DeliveredStatus("1")
}

object DeliveredStatus {

  def withValueOpt(value: String): Option[DeliveredStatus] = DeliveredStatus.values.find(_.value == value)
  def withValue(value: String): DeliveredStatus =
    withValueOpt(value).getOrElse(throw java.lang.IllegalArgumentException(s"DeliveredStatus enum case not found: $value"))

  given decoderDeliveredStatus: Decoder[DeliveredStatus] = Decoder.decodeString.map(withValue)
  given encoderDeliveredStatus: Encoder[DeliveredStatus] = Encoder.encodeString.contramap[DeliveredStatus](_.value)
