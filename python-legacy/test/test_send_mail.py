# coding: utf-8

"""
    Mail Baby API

    This is an API for accesssing the mail services.  # noqa: E501

    The version of the OpenAPI document: 1.0.0
    Contact: detain@interserver.net
    Generated by: https://openapi-generator.tech
"""


from __future__ import absolute_import

import unittest
import datetime

import openapi_client
from openapi_client.models.send_mail import SendMail  # noqa: E501
from openapi_client.rest import ApiException

class TestSendMail(unittest.TestCase):
    """SendMail unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional):
        """Test SendMail
            include_option is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # model = openapi_client.models.send_mail.SendMail()  # noqa: E501
        if include_optional :
            return SendMail(
                id = 56, 
                _from = openapi_client.models.mail_contact.MailContact(
                    email = 'johndoe@company.com', 
                    name = 'John Doe', ), 
                to = [
                    openapi_client.models.mail_contact.MailContact(
                        email = 'johndoe@company.com', 
                        name = 'John Doe', )
                    ], 
                subject = 'Your Package has been Delivered!', 
                body = 'The package you ordered on 2021-01-23 has been delivered. If the package is broken into many pieces, please blaim someone else.
', 
                replyto = [
                    openapi_client.models.mail_contact.MailContact(
                        email = 'johndoe@company.com', 
                        name = 'John Doe', )
                    ], 
                cc = [
                    openapi_client.models.mail_contact.MailContact(
                        email = 'johndoe@company.com', 
                        name = 'John Doe', )
                    ], 
                bcc = [
                    openapi_client.models.mail_contact.MailContact(
                        email = 'johndoe@company.com', 
                        name = 'John Doe', )
                    ], 
                attachments = [
                    openapi_client.models.mail_attachment.MailAttachment(
                        filename = 'company_logo.png', 
                        data = bytes(b'blah'), )
                    ]
            )
        else :
            return SendMail(
                id = 56,
                _from = openapi_client.models.mail_contact.MailContact(
                    email = 'johndoe@company.com', 
                    name = 'John Doe', ),
                to = [
                    openapi_client.models.mail_contact.MailContact(
                        email = 'johndoe@company.com', 
                        name = 'John Doe', )
                    ],
                subject = 'Your Package has been Delivered!',
                body = 'The package you ordered on 2021-01-23 has been delivered. If the package is broken into many pieces, please blaim someone else.
',
        )

    def testSendMail(self):
        """Test SendMail"""
        inst_req_only = self.make_instance(include_optional=False)
        inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
