...
class BlockingApi(
   ...
   * @return GenericResponse
   */
  def delistBlock(body: String): Option[GenericResponse] = {
    val await = Try(Await.result(delistBlockAsync(body), Duration.Inf))
    await match {
      case Success(i) => Some(await.get)
      case Failure(t) => None
    }
   ...
   * @return Future(GenericResponse)
   */
  def delistBlockAsync(body: String): Future[GenericResponse] = {
      helper.delistBlock(body)
  }
...
class BlockingApiAsyncHelper(client: TransportClient, config: SwaggerConfig) extends ApiClient(client, config) {
  ...
  def delistBlock(body: String)(implicit reader: ClientResponseReader[GenericResponse], writer: RequestWriter[String]): Future[GenericResponse] = {
    // create path and map variables
