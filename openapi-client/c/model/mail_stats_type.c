...
void mail_stats_type_free(mail_stats_type_t *mail_stats_type) {
    ...
    if (mail_stats_type->volume) {
        mail_stats_volume_free(mail_stats_type->volume);
...
cJSON *mail_stats_type_convertToJSON(mail_stats_type_t *mail_stats_type) {
    ...
    if(mail_stats_type->time != mailbaby_email_delivery_and_management_service_api_mail_stats_type_TIME_NULL) {
    ...
    if(mail_stats_type->volume) {
    cJSON *volume_local_JSON = mail_stats_volume_convertToJSON(mail_stats_type->volume);
...
mail_stats_type_t *mail_stats_type_parseFromJSON(cJSON *mail_stats_typeJSON){
    ...
    mail_stats_volume_t *volume_local_nonprim = NULL;
    ...
    mailbaby_email_delivery_and_management_service_api_mail_stats_type_TIME_e timeVariable;
    if (time) { 
    ...
    if (volume) { 
    volume_local_nonprim = mail_stats_volume_parseFromJSON(volume); //nonprimitive
    }
    ...
    if (volume_local_nonprim) {
        mail_stats_volume_free(volume_local_nonprim);
